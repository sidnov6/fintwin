...
export function PictureView({ state, lang, applyState, send, toast, addMessage }: PictureProps) {
  ...
  return <div className="page">
    ...
    {state.profile?.sampleLoaded && <div className="banner"><CircleAlert />{t.sampleNote}. {lang === "de" ? "Änderungen bleiben Teil dieser synthetischen Demonstration. Eigene Angaben gehören in einen neuen Haushalt." : "Edits remain part of this synthetic demonstration. Use a new household for your own figures."}</div>}
    ...
    <div className="grid-2">
      {GROUPS.map(group => {
        ...
        return <section className="panel" key={group}>
          ...
          <div>{visible.map(def => { const fact = facts[def.key]; return <div key={def.key}>
            ...
            {editing === def.key && <FactEditor factKey={def.key} lang={lang} initial={fact?.value} onSave={async value => { await actions.save(def.key, value); setEditing(null); toast(`${def.label[lang]}: ${fact?factDisplay(def.key,fact.value,lang):'—'} → ${factDisplay(def.key,value,lang)}`); }} onCancel={() => setEditing(null)} onRemove={fact ? async () => { await actions.remove(def.key); setEditing(null); } : undefined} />}
          </div>; })}</div>
        </section>;
      })}
      ...
      {portfolio && <section className="panel" style={{ gridColumn: "1 / -1" }}><h3>{t.portfolio}<span className="spacer" /><small>{lang==='de'?'Feste synthetische Beispielkurse':'Frozen synthetic sample quotes'} · {portfolio.asOf.slice(0,10)}</small></h3>
        ...
        <p className="note">{lang === "de" ? "Fiktives Beispieldepot mit eingefrorenen Beispielkursen. Keine Live-Marktdaten. Nicht für Handelsentscheidungen." : "Fictional portfolio with frozen sample quotes. Not live market data. Not for trading decisions."}</p></section>}
    </div>
