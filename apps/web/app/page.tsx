...
function Assistent() {
  ...
  const [input,setInput]=useState(""); const [loading,setLoading]=useState(false); const [listening,setListening]=useState(false); const [aiLive,setAiLive]=useState(false); const recognition=useRef<SpeechRecognitionLike|null>(null);
  ...
  function toggleVoice(){if(listening){recognition.current?.stop();setListening(false);return;}const w=window as typeof window & {SpeechRecognition?:new()=>SpeechRecognitionLike;webkitSpeechRecognition?:new()=>SpeechRecognitionLike};const Recognition=w.SpeechRecognition??w.webkitSpeechRecognition;if(!Recognition){setMessages(m=>[...m,{role:"assistant",text:"Die Spracherkennung wird in diesem Browser nicht unterstützt. Bitte nutzen Sie Chrome oder geben Sie Ihre Frage ein."}]);return;}const rec=new Recognition();recognition.current=rec;rec.lang="de-DE";rec.interimResults=false;rec.continuous=false;rec.onresult=e=>{const text=e.results[0][0].transcript;setInput(text);void ask(text);};rec.onend=()=>setListening(false);rec.onerror=()=>setListening(false);setListening(true);rec.start();}
  const suggestions=["Wofür geben wir am meisten aus?","Was bedeutet ein Zins von 6 %?","Was fehlt für das Beratungsgespräch?"];
  return <section className="assistant-page"><div className="assistant-intro"><p className="kicker dark"><Sparkles /> FinTwin KI-Assistent</p><h1>Fragen Sie Ihre Finanzen.<br /><em>Auf Deutsch. Mit Quellen.</em></h1><p>Text oder Sprache – Antworten basieren auf geprüften Daten Ihres Finanz-Twins.</p><div className={`ai-status ${aiLive?"live":"demo"}`}><span />{aiLive?"Live-KI verbunden":"Demo-Modus · API-Schlüssel nicht eingerichtet"}</div></div><div className="chat-card"><div className="chat-top"><span><Bot /> FinTwin</span><small><ShieldCheck /> Geschützter Planungsraum</small></div><div className="messages" aria-live="polite">{messages.map((m,i)=><div className={`message ${m.role}`} key={i}><span>{m.role==="assistant"?<Sparkles/>:"MB"}</span><div><p>{m.text}</p>{m.sources&&m.sources.length>0&&<details><summary><Database size={13}/> {m.sources.length} Quellen anzeigen</summary>{m.sources.slice(0,5).map(s=><code key={s}>{s}</code>)}</details>}</div></div>)}{loading&&<div className="typing"><i/><i/><i/></div>}</div><div className="suggestions">{suggestions.map(s=><button key={s} onClick={()=>{setInput(s);}}>{s}</button>)}</div><form className="composer" onSubmit={submit}><button type="button" className={`mic ${listening?"listening":""}`} onClick={toggleVoice} aria-label={listening?"Aufnahme beenden":"Sprachfrage starten"}>{listening?<MicOff/>:<Mic/>}</button><label><span className="sr-only">Ihre Frage</span><input value={input} onChange={e=>setInput(e.target.value)} placeholder={listening?"Ich höre zu …":"Frage eingeben …"}/></label><button type="submit" disabled={!input.trim()||loading} aria-label="Frage senden"><Send /></button></form><p className="voice-note"><Volume2 size={13} /> Antworten werden auf Wunsch vorgelesen. Keine Abschlüsse oder Produktempfehlungen.</p></div></section>;
}
